...
dict = np.array((["SYS", "R_AR"], ["SYS", "C_AR"], ["LV", "EA"], ["LV", "EB"],
                 ["LV", "TC"], ["LV", "TR"], ["LA", "tC"], ["RA", "tC"]))
problem = {
    'num_vars': 8,
    'names': ['R_AR_SYS', 'C_AR_SYS', 'EA_LV', 'EB_LV',
              'TC_LV', 'TR_LV', 'tC_LA', 'tC_RA'],
    'bounds': [[0.32, 0.96], [0.6, 1.8], [1.675, 5.025], [0.1, 0.3],
               [0.125, 0.375], [0.2, 0.6], [0.45, 1.35], [0.45, 1.35]]
}
num_parameters = problem['num_vars']
'''
# 14 means 252 training values
observation_index_points_ = saltelli.sample(problem, 14) # N*(2D+2) with D num_vars and N chosen
...
for i in range(num_parameters):
    ...
    def build_gp(amplitude, length_scale, observation_noise_variance):
        ...
        return tfd.GaussianProcess(kernel=kernel, index_points=np.array([observation_index_points_trans_.T[i]]).T, observation_noise_variance=observation_noise_variance)
...
np.savetxt('length_scales_8_parameters.csv', L, delimiter=',')
import pdb; pdb.set_trace()
'''
start = time.time()

# 6, 28, 56, 112
observation_index_points_ = saltelli.sample(problem, 6)  # N*(2D+2) with D num_vars and N chosen
num_observations = len(observation_index_points_)
...
with open("length_scales_8_parameters.csv") as file_name:
...
'''
predictive_index_points_ = saltelli.sample(problem, 500) # N*(2D+2) with D num_vars and N chosen
...
Z.T[8] = true_function_
np.savetxt('test_set_8_parameters.csv', Z, delimiter=',')
import pdb; pdb.set_trace()
'''
predictive_index_points_ = np.zeros(shape=(num_predictions, num_parameters))
true_function_ = np.zeros(shape=num_predictions)

with open("test_set_8_parameters.csv") as file_name:

...
def generate_data(index_points_, observation_noise_variance):
    observations_ = (fun(index_points_.T[0], index_points_.T[1], index_points_.T[2], index_points_.T[3], 0, 0, 0, 0, 0, 0, 0, 0)
                     + np.random.normal(loc=0, scale=np.sqrt(observation_noise_variance), size=(num_observations)))
...
problem = {
    'num_vars': 4,
    'names': ['mu_1', 'mu_2', 'mu_3', 'mu_4'],
    'bounds': [[-1, 1], [-1, 1], [-1, 1], [-1, 1]]
}
num_parameters = problem['num_vars']
'''
# 25 means 250 training values
observation_index_points_ = saltelli.sample(problem, 25) # N*(2D+2) with D num_vars and N chosen
...
np.savetxt('length_scales_4_parameters.csv', L, delimiter=',')
import pdb; pdb.set_trace()
'''
start = time.time()

# 10, 50, 100, 200
observation_index_points_ = saltelli.sample(problem, 100) # N*(2D+2) with D num_vars and N chosen
num_observations = len(observation_index_points_)
...
with open("length_scales_4_parameters.csv") as file_name:
...
'''
predictive_index_points_ = saltelli.sample(problem, 900) # N*(2D+2) with D num_vars and N chosen
true_function_ = fun(predictive_index_points_.T[0], predictive_index_points_.T[1], predictive_index_points_.T[2], 
                     predictive_index_points_.T[3], 0, 0, 0, 0, 0, 0, 0, 0)
...
Z.T[4] = true_function_
np.savetxt('test_set_4_parameters.csv', Z, delimiter=',')
import pdb; pdb.set_trace()
'''
predictive_index_points_ = np.zeros(shape = (num_predictions, num_parameters))
true_function_ = np.zeros(shape = num_predictions)

with open("test_set_4_parameters.csv") as file_name:

...
def generate_data(index_points_, observation_noise_variance):
    observations_ = (fun(index_points_.T[0], index_points_.T[1], index_points_.T[2], index_points_.T[3], index_points_.T[4],
                         index_points_.T[5], index_points_.T[6], index_points_.T[7], 0, 0, 0, 0) + np.random.normal(loc=0, scale=np.sqrt(observation_noise_variance), size=(num_observations)))
...
problem = {
    'num_vars': 8,
    'names': ['mu_1', 'mu_2', 'mu_3', 'mu_4', 'mu_5', 'mu_6', 'mu_7', 'mu_8'],
...
# 6, 28, 56, 112
observation_index_points_ = saltelli.sample(problem, 56) # N*(2D+2) with D num_vars and N chosen
num_observations = len(observation_index_points_)
